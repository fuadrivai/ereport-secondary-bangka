...
class Set_kd_kelompok extends CI_Controller {
	function __construct() {
        ...
        $this->d['url'] = "set_kd_kelompok";
        $this->d['idnya'] = "setkdkelompok";
        $this->d['nama_form'] = "f_kdkelompok";
    ...
    public function datatable($id) {
        ...
        $d_total_row = $this->db->query("SELECT 
                                        ...
                                        WHERE a.`id_guru` = '".$this->d['admkonid']."' AND a.`id_kelompok` = '".$pc_id[0]."' 
    ...
    public function edit($id) {
        $q = $this->db->query("SELECT *, 'edit' AS mode FROM t_kelompok_kd WHERE id = '$id'")->row_array();
    ...
    public function simpan() {
        ...
        if ($p['_mode'] == "add") {
            $this->db->query("INSERT INTO t_kelompok_kd (id_guru, id_kelompok, tingkat, no_kd, jenis, nama_kd, semester, mid_final) VALUES ('".$this->d['admkonid']."', '".$p['id_kelompok']."', '".$p['tingkat']."', '".$p['kode']."', '".$p['jenis']."', '".addslashes($p['nama'])."', '".$this->d['semester']."', '".$p['semester']."')");

            $d['status'] = "ok";
            $d['data'] = "Data berhasil disimpan";
        } else if ($p['_mode'] == "edit") {
            $this->db->query("UPDATE t_kelompok_kd SET id_kelompok = '".$p['id_kelompok']."', tingkat = '".$p['tingkat']."', no_kd = '".$p['kode']."', jenis = '".$p['jenis']."', mid_final = '".$p['semester']."', nama_kd = '".addslashes($p['nama'])."' WHERE id = '".$p['_id']."'");
    ...
    public function hapus($id) {
        $this->db->query("DELETE FROM t_kelompok_kd WHERE id = '$id'");
        $this->db->query("DELETE FROM t_nilai_kelompok WHERE id_kelompok_kd = '$id'");
    ...
    public function index() {
    	$this->d['mapel_diampu'] = $this->db->query("SELECT 
                                    ...
                                    GROUP BY a.`id_kelompok`, c.`tingkat`")->result_array();
