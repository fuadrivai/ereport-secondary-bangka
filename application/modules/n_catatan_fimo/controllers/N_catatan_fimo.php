...
class N_catatan_fimo extends CI_Controller
{
    ...
    public function simpan()
    {
        ...
        for ($i = 1; $i < $p['jumlah']; $i++) {
            ...
            if ($mode_form == "add") {
                $strq = "INSERT INTO t_catatan_fimo (id_siswa,ta,catatan_mid,catatan_final) VALUES ('$id_siswa','$tasm','$catatan','$catatan_final')";
            } else if ($mode_form == "edit") {
                $strq = "UPDATE t_catatan_fimo SET  catatan_mid = '" . $catatan . "', catatan_final = '" . $catatan_final . "' WHERE ta = '" . $tasm . "' AND id_siswa = '" . $id_siswa . "'";
            }
    ...
    public function import_nilai()
    {
        ...
        if ($file[$length - 1] == 'xlsx' || $file[$length - 1] == 'xls') {
            ...
            $arr_perdata = array();
            foreach ($data as $d) {
                $strq = "UPDATE t_catatan_fimo SET  catatan_mid = '" . $d['catatan_mid'] . "', catatan_final = '" . $d['catatan_final'] . "' WHERE ta = '" . $d['ta'] . "' AND id_siswa = '" . $d['id_siswa'] . "'";
                $this->db->query($strq);
            }
