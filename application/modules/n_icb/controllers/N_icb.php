...
class N_icb extends CI_Controller
{
    ...
    public function cetak($bawa, $tasm = 0)
    {
        ...
        $q_nilai_harian = $this->db->query("SELECT 
                                d.nama nmsiswa, a.id_mapel_kd, a.id_siswa, if(a.jenis='h',CONCAT(a.nilai,'//',b.nama_kd),a.nilai) nilai
                                FROM t_nilai_icb a
    ...
    public function import_nilai()
    {
        ...
        if ($file[$length - 1] == 'xlsx' || $file[$length - 1] == 'xls') {
            ...
            $strq = "REPLACE INTO t_nilai_icb (tasm, jenis, id_guru_mapel, id_mapel_kd, id_siswa, nilai) VALUES ";
            $arr_perdata = array();
            foreach ($data as $d) {
            ...
            redirect('n_icb/index/' . $id_guru_mapel);
        } else {
    ...
    public function ambil_siswa($kelas)
    {
        ...
        if ($jenis == "h") {
            $ambil_nilai = $this->db->query("SELECT
                                        ...
                                        FROM m_siswa b 
                                        INNER JOIN t_nilai_icb a ON a.id_siswa = b.id
            ...
            $ambil_nilai = $this->db->query("SELECT 
                        t_nilai_icb.id_siswa ids, m_siswa.nama, IFNULL(t_nilai_icb.nilai,0) nilai
                        FROM t_nilai_icb 
                        INNER JOIN t_guru_mapel ON t_nilai_icb.id_guru_mapel = t_guru_mapel.id
                        INNER JOIN m_siswa ON t_nilai_icb.id_siswa = m_siswa.id
                        WHERE t_guru_mapel.id = '$id_guru_mapel' 
                        AND t_nilai_icb.id_mapel_kd = '$id_kd' 
                        AND t_nilai_icb.jenis = '$jenis' 
                        AND t_nilai_icb.tasm = '" . $this->d['tasm'] . "' ORDER BY m_siswa.nama ASC")->result_array();
            ...
            */
            $ambil_nilai = $this->db->query("SELECT 
						...
						INNER JOIN t_guru_mapel ON t_nilai_icb.id_guru_mapel = t_guru_mapel.id
						INNER JOIN m_siswa ON t_nilai_icb.id_siswa = m_siswa.id
						WHERE t_guru_mapel.id = '$id_guru_mapel' AND t_nilai_icb.jenis = '$jenis' AND 
						t_nilai_icb.tasm = '" . $this->d['tasm'] . "' ORDER BY m_siswa.nama ASC")->result_array();
            //j($ambil_nilai);
            //exit;
            //echo $this->db->last_query();
            //exit;
    ...
    public function simpan_nilai()
    {
        ...
        foreach ($p['nilai'] as $s) {
            ...
            $cek = $this->db->query("SELECT id FROM t_nilai_icb WHERE id_guru_mapel = '" . $p['id_guru_mapel'] . "' AND id_mapel_kd = '" . $p['id_mapel_kd'] . "' AND id_siswa = '" . $p['id_siswa'][$i] . "' AND jenis = '" . $p['jenis'] . "' AND tasm = '" . $this->d['tasm'] . "'")->num_rows();
            //$queri[] = $this->db->last_query();
            //exit;
            if ($cek > 0) {
                $jumlah_sudah++;
                $this->db->query("UPDATE t_nilai_icb SET nilai = '$s' WHERE id_guru_mapel = '" . $p['id_guru_mapel'] . "' AND id_mapel_kd = '" . $p['id_mapel_kd'] . "' AND id_siswa = '" . $p['id_siswa'][$i] . "' AND jenis = '" . $p['jenis'] . "'");
                //$queri[] = "update";
            } else {
                $this->db->query("INSERT INTO t_nilai_icb (tasm,jenis, id_guru_mapel, id_mapel_kd, id_siswa, nilai) VALUES ('" . $this->d['tasm'] . "', '" . $p['jenis'] . "', '" . $p['id_guru_mapel'] . "', '" . $p['id_mapel_kd'] . "', '" . $p['id_siswa'][$i] . "', '" . $s . "')");
                //$queri[] = "add";
            }
            $i++;
        }
