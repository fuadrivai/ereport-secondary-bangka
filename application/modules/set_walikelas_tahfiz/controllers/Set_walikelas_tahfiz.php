...
class Set_walikelas_tahfiz extends CI_Controller {
	function __construct() {
        parent::__construct();
        $this->sespre = $this->config->item('session_name_prefix');

        $this->d['admlevel'] = $this->session->userdata($this->sespre.'level');
        $this->d['url'] = "set_walikelas_tahfiz";
    ...
    public function datatable() {
        ...
        $d_total_row = $this->db->query("SELECT id FROM t_walikelas_tahfiz a
    ...
    public function edit($id) {
        $q = $this->db->query("SELECT *, 'edit' AS mode FROM t_walikelas_tahfiz WHERE id = '$id'")->row_array();
    ...
    public function simpan() {
        ...
        if ($p['_mode'] == "add") {
            $cek = $this->db->query("SELECT id FROM t_walikelas_tahfiz WHERE id_kelas = '".$p['id_kelas']."' AND tasm = '".$this->d['tasm']."'")->num_rows();

            if ($cek > 0) {
                $d['status'] = "gagal";
                $d['data'] = "Kelas tersebut sudah ada walinya..";                
            } else {
                $this->db->query("INSERT INTO t_walikelas_tahfiz (tasm, id_guru, id_kelas) VALUES ('".$this->d['tasm']."', '".$p['id_guru']."', '".$p['id_kelas']."')");

                $d['status'] = "ok";
                $d['data'] = "Data berhasil disimpan";
            }
        } else if ($p['_mode'] == "edit") {
            $this->db->query("UPDATE t_walikelas_tahfiz SET id_kelas = '".$p['id_kelas']."', id_guru = '".$p['id_guru']."' WHERE id = '".$p['_id']."'");
    ...
    public function hapus($id) {
        $this->db->query("DELETE FROM t_walikelas_tahfiz WHERE id = '$id'");
    ...
    public function index() {
        ...
        $q_kelas = $this->db->query("SELECT * FROM m_kelas_tahfiz")->result_array();
        if (!empty($q_kelas)) {
