...
class N_preschool extends CI_Controller {
	function __construct() {
        parent::__construct();
        $this->sespre = $this->config->item('session_name_prefix');
        $this->d['admlevel'] = $this->session->userdata($this->sespre.'level');
        $this->d['admkonid'] = $this->session->userdata($this->sespre.'konid');
        $this->d['url'] = "n_preschool";
    ...
    public function import($bawa) {
        ...
        if (!empty($d_nilai)) {
            ...
            foreach ($d_nilai as $ke => $dn) {
                ...
                $catatan_preschool = str_replace('; ', '. ', implode("; ", $txt_desk)); 
    ...
    public function import_nilai() {
        ...
        if($file[$length -1] == 'xlsx' || $file[$length -1] == 'xls') {
            ...
            for ($b = $idx_baris_mulai; $b < $idx_baris_selesai; $b++) {
                ...
                $xy_nilai_catatan_mid = $_sheet->getCell($this->kolom_xl[($idx_mulai_uts)].$b)->getCalculatedValue();
            ...
            redirect('n_preschool/index/'.$id_guru_mapel);
    ...
    public function ambil_siswa($kelas) {
        ...
        if ($jenis == "h") {
            ...
            */
            $ambil_nilai = $this->db->query("SELECT 
						t_nilai.id_siswa ids, m_siswa.nama, IFNULL(t_nilai.catatan,0) nilai, IFNULL(t_nilai.catatan_mid,0) nilai_mid
						FROM t_nilai 
						INNER JOIN t_guru_mapel ON t_nilai.id_guru_mapel = t_guru_mapel.id
						INNER JOIN m_siswa ON t_nilai.id_siswa = m_siswa.id
